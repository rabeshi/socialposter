...
{
  "crons": [
    { "path": "/api/cron/generate-content", "schedule": "55 17 * * *" },
...
Vercel automatically sends `Authorization: Bearer $CRON_SECRET` to these routes; make sure `CRON_SECRET` is set in the project's environment variables. The Hobby-plan generation cron runs daily at 17:55 UTC (10:55 AM Pacific during daylight-saving time), but only actually generates a batch once `AutomationSettings.nextGenerationAt` has arrived. The two-day cadence remains controlled from the Automation page. Hobby cron invocation may be delayed within the following hour.
